...
```{r}
#"To be wise is to show care and thoughtful discretion with regard for the future."
#— first define what we want to know. We want to know what the drop in undergraduate enrollment has been between September 1 and today, September 22. We then want to search for reasons for why that drop may have occurred.
#—Because this is a causal question, we start with the infinite Preceptor's Tables and then must make wise decisions accordingly to manage the amount of data we have access to. We must drop duplicate rows, drop columns we don't care about, and define our estimand in terms of potential outcomes. This will get us started to create the model!
```


After we have a model, Temperance suggests we should:
```{r}
#Check our model to make sure that everything we want is included and that we are not leaving out an important peice of data. We should also look at the way the data may changge in the course of the project and ensure that all of the new data is incorporated so that everything is as up to date as possible.
```
...
```{r Scene 3}
sep_change <-
  full_join(sep_old, sep_new, by = c("id", "title", "department", "name"))
```
